...
def main():
    ...
    if rank == 0:
        ...
        # DISTRIBUSI LEBIH RATA (round-robin)
    ...
    # PREPROCESS + TF LOKAL
    # =========================
    local_docs_words = [preprocess(doc) for doc in local_docs]
    local_tf_list = [compute_tf(doc) for doc in local_docs_words]
    ...
    if rank == 0:
        ...
        for df_part in all_df:
            for word, count in df_part.items():
                global_df[word] = global_df.get(word, 0) + count
    ...
    if rank == 0:
        ...
        print("Detail tiap process:", [len(chunk) for chunk in chunks])
